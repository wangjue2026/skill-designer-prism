...
4. **Step 4: 布局结构与组件映射**（调用 `01-system-solution-design/④布局与结构/` 与 `03-design-assets/`）：
   * **取材顺序（硬性，不可颠倒）**：必须**先消费 Step 2 产出的《标杆细节复用清单》** —— 命中项直接还原（来源标 `案例X 还原`）➔ 再用 [`03-design-assets/`](../03-design-assets/) 的通用规范补齐硬指标（尺寸 / 色值 / 组件规范）➔ 清单中标记「待推导」的部分，才回到体验目标与设计发力点自行推导（来源标 `体验目标推导`）。**严禁跳过前两级直接自由发挥。**
   * 调取 `4.1-业务页面框架选型指引.md` 从 `page-templates/` 选取最贴切的业务框架套头（如监控大盘标准套头）—— **若 Step 2 清单中已有案例用过的框架套头，优先复用它**；
   * 调取 `4.2-业务能力与组件选型指引.md` 将业务能力清单逐一映射到底层组件，并注入硬性尺寸约束 —— **案例已用过的组件优先复用**。
   * **标注元素还原来源**：组件映射时须逐项标明来历（`经验 EP-xx 复用` / `案例X 还原` / `体验目标推导`），供 Step 5 汇总落表。
...
| **2 设计点思考** | ①~⑤ 全部 | ① 设计发力点（`L1` 方向级，1~3 条）<br>② 业务属性与严肃程度定性<br>③ 选定专家范式（`P-01~P-08`）<br>④ **《标杆细节复用清单》**（颜色 / 尺寸 / 组件 / 模板 / 交互态 + 出处）<br>⑤ 各要素选型结论 | 发力点须能追溯到痛点编号；清单每项须有出处 |
| **3 未来旅程** | ① 主场景清单 + 痛点旗帜<br>② 发力点 `L1` | ① 优化后各场景旅程流程图（Mermaid）<br>② **痛点翻转对照表**（`🚩Sx-Py` ➔ 优化动作 ➔ 旅程落点） | **每条痛点必须在旅程中有明确落点** |
| **4 布局与结构** | ① 旅程流程图（页面与状态清单）<br>② **《标杆细节复用清单》** | ① 选定页面框架套头（`TPL-xx`）<br>② **业务能力 ➔ 组件映射清单**（含资产规范 + 硬性指标 + **来源**）<br>③ 关键布局与交互细节（栅格 / 交互态 / 防呆）<br>④ 页面体验与异常保护规则 | 映射清单**每行必须有「来源」**；硬性指标不得为空 |
| **5 设计说明书** | ①~④ 全部 | 六章完整说明书（单文件 HTML） | 六章齐备，章节↔步骤映射无空缺 |
| **6 AI Coding** | 用户确认后的说明书 | ① 高保真 Demo<br>② **回测报告**（对照 Step 4 映射清单逐项核验） | 回测必须**引用 Step 4 的清单条目** |

### 二、三条交接铁律

> ⚠️ **不许跨级臆造**：若上游产物缺失或字段不全，**必须停下向上游追问 / 补齐**，严禁下游自行猜测填补。
> ⚠️ **不许产出不落地**：任何一步的产物若最终不进《设计说明书》或 Step 6 回测，即为无效工作。
> ⚠️ **每步必须回链**：下游引用上游内容时须保留其编号（痛点 `🚩S1-P1` / 经验 `EP-07` / 案例 `案例A / E1` / 套头 `TPL-01`），实现**端到端可追溯**。

### 三、收敛校验（设计说明书 = 唯一出口）

六章与来源步骤的映射如下；**任一章节空缺、或其来源步骤产物缺失，即判定为上游交接断裂，必须回补而非臆造**：

| 说明书章节 | 来源步骤 | 上游缺件时的处理 |
| :--- | :--- | :--- |
| 一、需求洞察与体验目标 | Step 1 | 回 Step 1 补齐角色 / 痛点 / 体验目标 |
| 二、业务定性与设计选型 | Step 2 | 回 Step 2 补齐类型定性 / 发力点 / 范式 |
| 三、优化后各场景任务旅程 | Step 3 | 回 Step 3 补齐旅程与痛点翻转对照 |
| 四、页面布局结构与组件清单 | Step 4（含 Step 2 的《标杆细节复用清单》） | 回 Step 2 / 4 补齐清单与来源标注 |
| 五、关键界面布局与交互细节说明 | Step 4 | 回 Step 4 补齐栅格 / 交互态 / 防呆 |
