...
2. **Step 2: 设计思考、发力点提炼与标杆经验传承**（调用 `01-system-solution-design/②设计点思考/` 与 `03-design-assets/patterns/`）：
   * **先做设计思考与重要发力点提炼（标杆学习的前置动作）**：调取 [`2.1-设计思考与发力点.md`](../01-system-solution-design/②设计点思考/2.1-设计思考与发力点.md)，承接步骤 1 的场景痛点与体验目标，深入思考**本次需求的重要设计点在哪儿、核心矛盾是什么**，按四步法提炼出 1~3 个**设计发力方向（L1 方向级）**，并确立在标杆案例学习时的**侧重导向**；
   * **带着侧重点深入标杆案例库查阅与传承（查阅 2.2）**：调取 [`2.2-标杆案例库.md`](../01-system-solution-design/②设计点思考/2.2-标杆案例库.md) 与对应分册（如 Type-07），排查【通用设计场景清单】，并带着发力点的侧重靶向提取成熟经验（`EP-xx`，如 Type-07 的 11 项经验）与组件样式，输出结构化传承矩阵；
   * **匹配专家范式与元设计机制**：到设计资产库 [`patterns/patterns-cheatsheet.md`](../03-design-assets/patterns/patterns-cheatsheet.md) 反查能承载发力方向的 L2 专家范式（`P-01 ~ P-08`）与元机制；
   * **产出《标杆细节复用清单》（Step 4 的硬输入）**：从标杆经验中提取具体细节（颜色 Token、尺寸硬指标、组件选型、页面模板、关键交互态）汇总成清单，未命中项标「待推导」；
