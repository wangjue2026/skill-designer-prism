...
<div class="wrap">
  <div class="doc">
    ...
    <div style="background:#F8FAFC;border-left:3px solid var(--blue);padding:8px 12px;margin-bottom:12px;font-size:12px;color:#475569;">
      💡 <b>设计思路说明</b>：在挑选具体组件和样式前，先明确这套功能<b>最关键的体验卡点在哪里</b>，然后带着问题去标杆库里寻找成熟的现成解法，不做无意义的样式堆砌。
    </div>
    <ul>
      <li><b>核心设计原则（页面分工界限）</b>：[用大白话说清核心原则或页面分工，例如：首页管任务、详情页查明细，边界清晰不混淆]</li>
    </ul>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td><b>[设计点 1: 如规则配置防呆与覆盖范围可见]</b></td>
          <td>S1 / <span class="flag">🚩 S1-P1</span></td>
          <td>简单防呆</td>
          <td>参考 XDR/SASE 的轻量抽屉快速配置 (EP-01) 与目标即时测算 (EP-02)</td>
          <td>配置规则繁琐易错 ➔ 预置库 30 秒装配，覆盖终端大数即时可见</td>
        </tr>
        <tr>
          <td><b>[设计点 2: 如执行进度透明与卡死任务自动跳过]</b></td>
          <td>S2 / <span class="flag">🚩 S2-P2</span></td>
          <td>进度透明</td>
          <td>参考卡底细进度条 (EP-06) 与离线超时自动跳过机制</td>
          <td>黑盒盲等超时卡死 ➔ 细进度条实时可见，超时自动跳过不卡死</td>
        </tr>
        <tr>
          <td><b>[设计点 3: 如就地抽屉排查与分级处置]</b></td>
