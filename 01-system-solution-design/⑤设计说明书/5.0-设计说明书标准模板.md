...
<div class="wrap">
  <div class="doc">
    ...
    <div style="background:#F8FAFC;border-left:3px solid var(--blue);padding:8px 12px;margin-bottom:12px;font-size:12px;color:#475569;">
      💡 <b>设计推导说明</b>：设计发力点直接源自第一章的体验目标与用户核心诉求。先明确面对管理者的体验卡点如何从交互与心智层面破局，再带着发力方向去标杆库（第 4 节）挑选成熟机制与组件（EP-xx）进行工程承接。
    </div>
    <ul>
      <li><b>核心架构原则（页面职责分工）</b>：[用清晰专业的话说清页面分工，例如：首页只管「任务」的宏观大盘与调度、详情页才看「终端」的单机排查与处置，两者父子分明不混淆]</li>
    </ul>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>目标 1: 任务配置省时防呆 (S1 / <span class="flag">🚩 S1-P1</span>)</td>
          <td>[描述用户配置时的实际担心，如怕手工拼错参数、怕选错部门误伤核心电脑]</td>
          <td>[说明如何通过模块化预置库简化输入，并在保存前实时计算出覆盖终端总量]</td>
          <td>参考单列卡片 (EP-01)、抽屉轻量配置 (EP-03)、范围即时测算 (EP-04)</td>
        </tr>
        <tr>
          <td><b>[发力点 2: 长耗时任务进度连续反馈与离线自动容错]</b></td>
          <td>目标 2: 检测进度直观透明 (S2 / <span class="flag">🚩 S2-P2</span>)</td>
          <td>[描述用户等待时的不可预期焦虑，如缺乏执行反馈、怕离线设备卡死整个任务]</td>
          <td>[说明如何通过细进度条提供持续反馈，并对离线失联设备建立超时自动跳过机制]</td>
          <td>参考卡底细进度条 (EP-06)、运行态操作互斥锁定、离线超时自动跳过</td>
        </tr>
        <tr>
          <td><b>[发力点 3: 违规终端就地深度排查与轻重分级闭环处置]</b></td>
          <td>目标 3: 排查处置高效闭环 (S3, S4 / <span class="flag">🚩 S3-P3</span>, <span class="flag">🚩 S4-P4</span>)</td>
          <td>[描述排查处置脱节与跳页断流，如跳到全量日志排查原因、切换多控制台处置断网]</td>
          <td>[说明如何通过右侧抽屉就地展开违规原因，并提供轻提醒与强管控分级处置通道]</td>
          <td>参考头部一句话大结论 (EP-07)、右侧抽屉就地排查 (EP-09)、零信任联动隔离 (EP-10)</td>
        </tr>
      </tbody>
    </table>
