...
<div class="wrap">
  <div class="doc">
    ...
    <h3>2. 核心设计思考与发力点提炼 (源自第一章体验目标)</h3>
    <div style="background:#F8FAFC;border-left:3px solid var(--blue);padding:8px 12px;margin-bottom:12px;font-size:12px;color:#475569;">
      💡 <b>推导脉络说明</b>：设计发力点<b>并非直接从标杆库拼凑，而是从第一章的「体验目标」与用户心理痛点中深度提炼出的系统级体验策略</b>。先明确在心智模型和交互链路上如何破局，再带着战略发力点去标杆库（第 4 节）针对性挑选成熟机制与组件（EP-xx）进行工程承接。
    </div>
    <ul>
      <li><b>核心架构原则（页面职责界限）</b>：[用大白话说清核心原则或页面分工，例如：首页管任务大盘与周期调度、详情页查终端证据链与排障闭环，两者父子分明不混淆]</li>
    </ul>
    ...
    <table>
      <thead>
        <tr>
          <th>战略级设计发力点 (源自体验目标)</th>
          <th>承接的体验目标与场景</th>
          <th>用户心理矛盾与现状阻力</th>
          <th>系统级体验破局策略 (怎么破局)</th>
          <th>向标杆库靶向提取的解法机制</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>[发力点 1: 影响面确定性与零心智负担装配]</b></td>
          <td>体验目标 1: 任务配置省时防呆 (S1 / <span class="flag">🚩 S1-P1</span>)</td>
          <td>[描述用户配置时的心理顾虑，如怕配错炸网、盲配无底]</td>
          <td>[说明如何通过配置即所见、影响全透明实现系统破局]</td>
          <td>参考轻量抽屉表单 (EP-01)、目标范围即时测算 (EP-02) 等</td>
        </tr>
        <tr>
          <td><b>[发力点 2: 长耗时异步作业的掌控感与白盒化]</b></td>
          <td>体验目标 2: 进度直观透明 (S2 / <span class="flag">🚩 S2-P2</span>)</td>
          <td>[描述用户等待时的焦虑与失控感，如黑盒盲等、怕死信卡死]</td>
          <td>[说明如何通过全链路白盒感知与异常自动自愈实现破局]</td>
          <td>参考细进度条动态感知 (EP-04)、超时熔断自愈等</td>
        </tr>
        <tr>
          <td><b>[发力点 3: 根因证据链下钻与原地低阻力闭环]</b></td>
          <td>体验目标 3: 违规排查顺手处置 (S3, S4 / <span class="flag">🚩 S3-P3</span>, <span class="flag">🚩 S4-P4</span>)</td>
          <td>[描述用户排障与处置时的断流摩擦，如跨页查日志、处置脱节]</td>
          <td>[说明如何通过证据链原地展开与一键联动处置实现破局]</td>
          <td>参考 In-situ 抽屉排障 (EP-07)、零信任网络动态阻断 (EP-10) 等</td>
        </tr>
      </tbody>
    </table>
