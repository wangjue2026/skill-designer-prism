...
<div class="wrap">
  <div class="doc">
    ...
    <div style="background:#F8FAFC;border-left:3px solid var(--blue);padding:8px 12px;margin-bottom:12px;font-size:12px;color:#475569;">
      💡 <b>说明</b>：发力点直接来自第一章的体验目标。先明确针对用户的实际痛点我们打算怎么解决，然后再去标杆库挑选对应的成熟组件来落地。
    </div>
    <ul>
      <li><b>页面分工原则</b>：[用大白话说清页面分工，例如：首页只管「任务」，详情页才看「终端与明细」，两者分清不混淆]</li>
    </ul>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td><b>[发力点 1: 配置简单防呆，提前知道测多少台电脑]</b></td>
          <td>目标 1: 任务配置防呆 (S1 / <span class="flag">🚩 S1-P1</span>)</td>
          <td>[描述用户配置时的实际担心，如怕手写拼错、怕选错部门误伤业务电脑]</td>
          <td>[说明如何做成预置勾选、直接算出影响多少台电脑]</td>
          <td>参考单列卡片 (EP-01)、抽屉新建 (EP-03)、范围即时测算 (EP-04)</td>
        </tr>
        <tr>
          <td><b>[发力点 2: 检测进度实时可见，卡死能自动跳过]</b></td>
          <td>目标 2: 进度透明不盲等 (S2 / <span class="flag">🚩 S2-P2</span>)</td>
          <td>[描述用户等待时的焦虑，如不知道跑完没有只能盲等、怕离线电脑卡死整个任务]</td>
          <td>[说明如何通过细进度条直观展示进度，离线电脑自动超时跳过]</td>
          <td>参考卡底细进度条 (EP-06)、离线超时自动跳过机制</td>
        </tr>
        <tr>
          <td><b>[发力点 3: 查问题不跳页，原地直接看原因并处理]</b></td>
          <td>目标 3: 排查处置就地闭环 (S3, S4 / <span class="flag">🚩 S3-P3</span>, <span class="flag">🚩 S4-P4</span>)</td>
          <td>[描述用户排查时的繁琐断流，如跳到全量日志查原因、再线下找人或去别的系统操作断网]</td>
          <td>[说明如何在右侧抽屉直接看违规软件和进程证据，原地一键发通知或隔离]</td>
          <td>参考抽屉就地排查 (EP-09)、零信任网络隔离联动 (EP-10)</td>
        </tr>
      </tbody>
    </table>
