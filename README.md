...
> **核心主线**：涵盖 **【方案系统设计 (1~5 步)】** 与 **【设计细节调优 (b~c 步)】** 双轨执行模型。
...
graph TD
    ...
    subgraph MacroPath ["方案系统设计主线 (1~5 步闭环)"]
        ...
        M3["3. 布局结构与组件映射 (④布局与结构/)"]
        M4["4. 完整设计说明书交付 (⑤设计说明书/)"]
...
│
├── 01-system-solution-design/                       # 【方案系统设计主线 (1~5 步)】
│   ├── ①需求理解与分析/                            # 步骤 1：需求理解与分析 (精简为 2 篇核心指南)
│   │   ├── 1.1-需求理解与现状旅程.md        # 涵盖现状痛点穿透 + 目标用户主要任务旅程 (JTBD)
│   │   └── 1.2-体验目标.md                  # 涵盖体验收益与务实版体验目标推导指南 (去口号化)
│   ├── ②设计点思考/                                # 步骤 2：设计点思考与标杆经验匹配
│   │   └── 2.1-设计思考与发力点.md  # 2.1 范式大类分类目录、定性矩阵与核心解题机制
│   ├── ③未来旅程/                                  # 原步骤 3：已从主线移除 (留档备查)
│   │   └── 3.1-未来旅程.md             # 3.1 本质任务旅程与 Mermaid 规划流程图 (历史资产)
│   ├── ④布局与结构/                                # 步骤 3：布局结构与组件映射
│   │   ├── 4.1-业务页面框架选型指引.md # 步骤 3 页面清单推导总览与交付模板
│   │   └── 4.2-业务能力与组件选型指引.md           # 4.2 业务能力 ➔ 组件硬指标映射清单
│   ├── ⑤设计说明书/                                # 步骤 4：生成完整设计说明书
│   │   └── 5.0-设计说明书标准模板.md              # 5.0 标准设计说明书模板 (五章固定 · 交付一句话确认)
│   └── ⑥AI coding 要求/                             # 步骤 5：前端 Demo 输出与回测
...
## 🚀 核心协同铁律
1. **以说明书为共识契约**：在做方案系统设计时，织雨输出第 4 步《设计说明书》后必须主动停下来等待用户确认，绝不擅自偷跑第 5 步代码；
2. **转轨主动预警**：细节调优一旦触及结构栅格崩坏或任务流断裂，必须主动建议升级为方案系统设计；
3. **闭环回测验证**：代码落地必须 100% 反向核对第 3 步列出的业务套头与组件清单；
